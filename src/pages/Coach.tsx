...
function StudyCard({ card }: { card: ParsedStudyCard }) {
    ...
    <article className="w-full max-w-2xl rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Frage</p>
          <p className="mt-2 text-base font-semibold leading-7 text-slate-950 dark:text-white">{card.question}</p>
        </div>
      </div>

      <label className="mt-4 block">
        <span className="text-sm font-medium text-slate-600 dark:text-slate-300">Deine Antwort</span>
        <textarea
          value={answerDraft}
          onChange={(event) => setAnswerDraft(event.target.value)}
          className="mt-2 min-h-24 w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none transition focus:border-teal-500 focus:bg-white dark:border-slate-800 dark:bg-slate-900 dark:focus:bg-slate-950"
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        >
          <Eye size={16} />
          {showAnswer ? "Lösung ausblenden" : "Lösung zeigen"}
        </button>
        {answerDraft.trim() ? (
      ...
      {showAnswer ? (
        <div className="mt-4 rounded-2xl bg-teal-50 px-4 py-3 text-sm leading-6 text-slate-800 dark:bg-teal-500/10 dark:text-slate-100">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-teal-700 dark:text-teal-300">Musterantwort</p>
    ...
    return (
      <div className="space-y-3">
        {cards.map((card, index) => (
          <StudyCard key={`${card.question}-${index}`} card={card} />
        ))}
      </div>
    );
  }
...
export function CoachPage() {
  ...
  const [messages, setMessages] = useState<CoachChatMessage[]>([
    {
      role: "assistant",
      content: "Womit soll ich dir helfen?"
    }
  ]);
  ...
  return (
    <div className="grid min-h-[calc(100vh-8rem)] gap-6 xl:grid-cols-[18rem_1fr]">
      <aside className="space-y-5 rounded-[32px] border border-white/50 bg-white/80 p-5 shadow-panel dark:border-slate-800 dark:bg-slate-900/80">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">AI Trainer</p>
          <h3 className="mt-2 font-display text-2xl text-slate-950 dark:text-white">Coach Chat</h3>
        </div>
        ...
        <div className="rounded-3xl border border-slate-200/80 p-4 dark:border-slate-800">
          <p className="text-sm font-semibold text-slate-950 dark:text-white">Kontext</p>
          <div className="mt-3 grid grid-cols-2 gap-2 text-sm text-slate-500">
            <span>{activeExams.length} Klausuren</span>
      ...
      <section className="flex min-h-[34rem] flex-col overflow-hidden rounded-[32px] border border-white/50 bg-white/80 shadow-panel dark:border-slate-800 dark:bg-slate-900/80">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200/70 px-5 py-4 dark:border-slate-800">
          ...
          <button
            onClick={() => {
              setMessages([{ role: "assistant", content: "Womit soll ich dir helfen?" }]);
              setError(undefined);
              setSource(undefined);
            }}
